...
(function () {
  ...
  async function loadBlockyStatus() {
    try {
      ...
      if (status?.installed) {
        ...
        if (configureBtn) window.HostBerry&&window.HostBerry.setVisible(configureBtn, true, 'inline-block');

        if (status?.active) {
          indicator.className = 'status-indicator status-online';
          statusText.textContent = t('blocky.active', 'Active');
          if (enableBtn) enableBtn.classList.add('d-none');
          if (disableBtn) window.HostBerry&&window.HostBerry.setVisible(disableBtn, true, 'inline-block');
          if (refreshBtn) window.HostBerry&&window.HostBerry.setVisible(refreshBtn, true, 'inline-block');
          if (blockingRow) window.HostBerry&&window.HostBerry.setVisible(blockingRow, true);
          if (blockingValue) {
          ...
          if (enableBtn) window.HostBerry&&window.HostBerry.setVisible(enableBtn, true, 'inline-block');
          if (disableBtn) disableBtn.classList.add('d-none');
          if (refreshBtn) refreshBtn.classList.add('d-none');
          if (blockingRow) blockingRow.classList.add('d-none');
        }
      } else {
        indicator.className = 'status-indicator status-offline';
        statusText.textContent = t('blocky.not_installed', 'Not installed');
        if (installBtn) window.HostBerry&&window.HostBerry.setVisible(installBtn, true, 'inline-block');
    ...
    document.getElementById('blockyConfigForm')?.addEventListener('submit', async function (e) {
      ...
      try {
        ...
        if (resp && resp.ok && result?.success !== false) {
          ...
          document.getElementById('blocky-config-form').classList.add('d-none');
        } else {
          notify('danger', result?.error || t('errors.operation_failed', 'Operation failed'));
        }
      } catch (error) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('blocky-install-btn')?.addEventListener('click', installBlocky);
    document.getElementById('blocky-configure-btn')?.addEventListener('click', showBlockyConfig);
    document.getElementById('blocky-enable-btn')?.addEventListener('click', enableBlocky);
    document.getElementById('blocky-disable-btn')?.addEventListener('click', disableBlocky);
    document.getElementById('blocky-refresh-lists-btn')?.addEventListener('click', blockyRefreshLists);
    document.getElementById('blocky-show-config-btn')?.addEventListener('click', showBlockyConfig);
    loadBlockyStatus();
    buildPresetListsUI();
    bindForms();
