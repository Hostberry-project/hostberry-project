...
(function() {
    ...
    const checkSecurityWarnings = async () => {
        try {
            const resp = await apiRequestFn('/api/v1/auth/me');
            if (!resp.ok) return;
            const user = await resp.json();
            if (!user || !user.default_password_in_use) return;
            if (document.getElementById('default-password-alert')) return;
            const msg = t('dashboard.security_default_password', 'La cuenta admin sigue usando la contraseña por defecto. Cámbiala en Ajustes.');
            const title = t('dashboard.security_alert_title', 'Alerta de seguridad');
            const settings = t('navigation.settings', 'Ajustes');
            const alert = document.createElement('div');
            alert.id = 'default-password-alert';
            alert.className = 'alert alert-danger alert-dismissible fade show mb-4';
            alert.setAttribute('role', 'alert');
            alert.innerHTML = '<i class="bi bi-shield-exclamation me-2"></i><strong>' + escapeHtml(title) + ':</strong> ' +
                escapeHtml(msg) + ' <a href="/settings" class="alert-link ms-1">' + escapeHtml(settings) + '</a>' +
