...
(function () {
      ...
      logs.forEach((log) => {
        ...
        item.innerHTML = `
          <div class="d-flex align-items-center gap-2 flex-wrap">
            <span class="text-white-50 small hb-minw-80">${timeStr}</span>
            <span class="text-white-50 small hb-minw-90">${dateStr}</span>
  ...
  async function restoreBackup() {
    try {
      const listResp = await api('/api/v1/system/backups');
      const listPayload = await listResp?.json?.().catch(() => ({}));
      const backups = listPayload?.backups || [];
      if (!backups.length) {
        toast('warning', t('system.restore_none', 'No backups available'));
        return;
      }
      const choice = prompt(
        t('system.restore_pick', 'Enter backup filename to restore') + ':\n' + backups.join('\n'),
        backups[0]
      );
      if (!choice) return;
      if (!confirm(t('system.restore_confirm', 'Restore backup? Service restart required.'))) return;
      toast('info', t('system.restore_running', 'Restoring...'));
      const resp = await api('/api/v1/system/restore', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ file: choice.trim() }),
        sourceElement: document.getElementById('action-restore')
      });
      const payload = await resp?.json?.().catch(() => ({}));
      if (!resp || !resp.ok || payload?.success === false) {
        toast('danger', payload?.error || t('system.restore_error', 'Restore failed'));
        return;
      }
      toast('success', payload?.message || t('system.restore_success', 'Restore completed'));
    } catch (error) {
      if (window.HostBerry && HostBerry.reportError) {
        HostBerry.reportError('restore', error, t('system.restore_error', 'Restore failed'));
      } else {
        toast('danger', t('system.restore_error', 'Restore failed'));
      }
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('action-restore')?.addEventListener('click', restoreBackup);
