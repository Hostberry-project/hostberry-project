...
(function() {
  ...
  function updateStatusUI(enabled, running) {
    ...
    if (statusBar) {
      window.HostBerry&&window.HostBerry.setProgress(statusBar, isActuallyActive ? '100%' : '0%'.replace('%',''));
  ...
  async function loadAccessPoints() {
    ...
    container.innerHTML = '<div class="text-center py-4"><div class="spinning mb-3"><i class="bi bi-arrow-clockwise" class="hb-icon-2rem"></i></div><p class="text-muted">' + t('common.loading', 'Loading...') + '</p></div>';
    ...
    try {
      ...
      if (resp && resp.ok) {
        ...
        if (!Array.isArray(aps) || aps.length === 0) {
          container.innerHTML = `
            <div class="text-center py-4">
              <i class="bi bi-wifi text-muted" class="hb-icon-3rem"></i>
        ...
        if (hasNotTransmitting) {
          ...
          if (diagnosticsSection) {
            window.HostBerry&&window.HostBerry.setVisible(diagnosticsSection, true);
            loadDiagnostics();
          }
        } else {
          const diagnosticsSection = document.getElementById('diagnosticsSection');
        ...
        container.innerHTML = `
          <div class="text-center py-4">
            <i class="bi bi-exclamation-triangle text-warning" class="hb-icon-3rem"></i>
      ...
      container.innerHTML = `
        <div class="text-center py-4">
          <i class="bi bi-exclamation-triangle text-danger" class="hb-icon-3rem"></i>
    ...
    try {
      ...
      if (resp && resp.ok) {
        ...
        if (!Array.isArray(clients) || clients.length === 0) {
          container.innerHTML = `
            <div class="text-center py-4">
              <i class="bi bi-people text-muted" class="hb-icon-3rem"></i>
    ...
    try {
      ...
      if (resp && resp.ok) {
        ...
        if (diagnostics.recent_logs) {
          ...
          html += '<pre class="bg-dark text-light p-3 rounded" class="hb-pre-scroll">' + escapeHtml(diagnostics.recent_logs.substring(0, 2000)) + '</pre>';
          html += '</div>';
        }
  ...
  function initHostAPDPage() {
    ...
    waitForHostBerry(async () => {
      document.getElementById('refresh-hostapd-status-btn')?.addEventListener('click', loadHostAPDStatus);
      document.getElementById('restart-hostapd-btn')?.addEventListener('click', restartHostAPD);
      document.getElementById('refresh-access-points-btn')?.addEventListener('click', loadAccessPoints);
      document.getElementById('load-diagnostics-btn')?.addEventListener('click', loadDiagnostics);
      document.getElementById('refresh-clients-btn')?.addEventListener('click', loadClients);
      document.getElementById('toggle-hostapd-btn')?.addEventListener('click', toggleHostAPD);
      document.getElementById('toggle-hostapd-password-btn')?.addEventListener('click', toggleHostAPDPassword);
