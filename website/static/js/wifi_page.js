...
(function() {
  ...
  function updateStatusCards(data) {
    ...
    if (wifiStatusValue && wifiStatusBar && wifiStatusIcon) {
      ...
      window.HostBerry&&window.HostBerry.setProgress(wifiStatusBar, enabled ? '100%' : '0%'.replace('%',''));
    ...
    if (signalValue && signalBar) {
      ...
      window.HostBerry&&window.HostBerry.setProgress(signalBar, signalPercent + '%'.replace('%',''));
    }
  ...
  async function scanNetworks() {
    ...
    if (loadingEl) window.HostBerry&&window.HostBerry.setVisible(loadingEl, true);
    ...
    try {
      ...
      if (!resp || !resp.ok) {
        ...
        if (emptyEl) {
          window.HostBerry&&window.HostBerry.setVisible(emptyEl, true);
          emptyEl.innerHTML = `
            <div class="text-center py-5">
              <i class="bi bi-exclamation-triangle text-warning hb-icon-4rem"></i>
              ...
              <button type="button" class="btn btn-primary mt-3" data-action="scan-networks">
            ...
            if (emptyEl) {
              ...
              emptyEl.innerHTML = `
                <div class="text-center py-5">
                  ...
                  <button type="button" class="btn btn-primary mt-3" data-action="reload-page">
      ...
      if (networks.length === 0) {
        if (emptyEl) window.HostBerry&&window.HostBerry.setVisible(emptyEl, true);
        if (tableEl) tableEl.classList.add('d-none');
      } else {
        if (emptyEl) emptyEl.classList.add('d-none');
        if (tableEl) window.HostBerry&&window.HostBerry.setVisible(tableEl, true);
        if (tbody) {
          tbody.innerHTML = '';
      ...
      if (emptyEl) {
        ...
        emptyEl.innerHTML = `
          <div class="text-center py-5">
            <i class="bi bi-exclamation-triangle text-danger hb-icon-4rem"></i>
            ...
            <button class="btn btn-primary mt-3" data-action="scan-networks">
              <i class="bi bi-arrow-clockwise me-2"></i>${t('common.retry', 'Retry')}
            </button>
          </div>
        `;
  ...
  document.addEventListener('DOMContentLoaded', async function() {
    document.getElementById('refresh-wifi-status')?.addEventListener('click', loadConnectionStatus);
    ...
    document.getElementById('unblock-wifi-btn')?.addEventListener('click', unblockWiFi);
    document.getElementById('toggle-software-switch-btn')?.addEventListener('click', toggleSoftwareSwitch);
    document.body.addEventListener('click', (e) => {
      if (e.target.closest('[data-action="scan-networks"]')) scanNetworks();
      if (e.target.closest('[data-action="reload-page"]')) location.reload();
    });
    ...
    try {
      ...
      if (resp && resp.ok) {
        ...
        if (unblockBtn && data.hard_blocked) {
          window.HostBerry&&window.HostBerry.setVisible(unblockBtn, true);
        }
      }
    } catch (error) {
      console.error(t('wifi.status_error', 'Error getting WiFi status') + ':', error);
