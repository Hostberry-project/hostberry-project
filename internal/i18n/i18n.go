...
func Init(localesPath string) error {
	...
	for _, lang := range manager.supportedLanguages {
		...
		if err := manager.loadLanguage(lang, langFile); err != nil {
			log.Printf("Advertencia: no se pudo cargar %s: %v", langFile, err)
		}
	}
...
func formatTranslatedLog(key string, args ...interface{}) string {
	if manager == nil {
		if len(args) > 0 {
			return fmt.Sprint(append([]interface{}{key}, args...)...)
		}
		return key
	}
...
func LogT(key string, args ...interface{}) {
	log.Print(formatTranslatedLog(key, args...))
}
