...
func stopWpaSupplicant(interfaceName string) {
	...
	utils.ExecuteCommand(fmt.Sprintf("sudo pkill -f 'wpa_supplicant.*-i.*%s' 2>/dev/null || true", interfaceName))
	...
	for i := 0; i < 5; i++ {
		...
		if i == 4 {
			...
			utils.ExecuteCommand(fmt.Sprintf("sudo pkill -9 -f 'wpa_supplicant.*%s' 2>/dev/null || true", interfaceName))
			utils.ExecuteCommand("sudo killall -9 wpa_supplicant 2>/dev/null || true")
		}
	}

	for _, dir := range WpaSocketDirs {
		utils.ExecuteCommand(fmt.Sprintf("sudo rm -f %s/%s 2>/dev/null || true", dir, interfaceName))
	}
}
