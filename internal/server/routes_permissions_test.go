...
func TestOperatorCanAccessMonitoringRoutes(t *testing.T) {
	app := setupPermissionTestApp(t)
	token := createTestUserToken(t, "operator")

	tests := []struct {
		method string
		path   string
	}{
		{method: http.MethodGet, path: "/api/v1/system/stats"},
		{method: http.MethodGet, path: "/api/v1/network/status"},
		{method: http.MethodGet, path: "/api/v1/hostapd/access-points"},
		{method: http.MethodGet, path: "/api/v1/tor/status"},
		{method: http.MethodGet, path: "/api/v1/adblock/status"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.method+" "+tt.path, func(t *testing.T) {
			...
			if resp.StatusCode == http.StatusForbidden || resp.StatusCode == http.StatusUnauthorized {
				t.Fatalf("operator should access monitoring route, got %d", resp.StatusCode)
			}
		})
	}
}

func TestOperatorBlockedFromDestructiveRoutes(t *testing.T) {
	app := setupPermissionTestApp(t)
	token := createTestUserToken(t, "operator")
	...
	tests := []struct {
		...
		{method: http.MethodPost, path: "/api/v1/system/restart"},
		{method: http.MethodPost, path: "/api/v1/system/backup"},
		{method: http.MethodPost, path: "/api/v1/wifi/connect"},
		{method: http.MethodPost, path: "/api/v1/tor/enable"},
		{method: http.MethodPost, path: "/api/v1/adblock/enable"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.method+" "+tt.path, func(t *testing.T) {
			...
			if resp.StatusCode != http.StatusForbidden {
				t.Fatalf("operator should be forbidden, got %d", resp.StatusCode)
			}
		})
	}
}
