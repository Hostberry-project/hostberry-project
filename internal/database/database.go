...
func Init() error {
	...
	if cfg.Database.Type == "sqlite" {
		_ = os.Chmod(cfg.Database.Path, 0600)
	}
	...
	if err := migrateOnboardingFlagsOnce(); err != nil {
		return fmt.Errorf("error migrando flags de onboarding: %v", err)
	}
...
const (
	onboardingLegacyMigratedKey = "onboarding_legacy_migrated_v1"
	setupWizardCompletedByAPIKey = "setup_wizard_completed_by_api"
)

// migrateOnboardingFlagsOnce aplica la migración legacy una sola vez y repara estados inconsistentes.
func migrateOnboardingFlagsOnce() error {
	if val, err := GetConfig(onboardingLegacyMigratedKey); err == nil && val == "1" {
		return repairInconsistentOnboardingFlags()
	}

	var legacyCount int64
	if err := DB.Model(&models.User{}).Where("login_count >= ?", 3).Count(&legacyCount).Error; err != nil {
		return err
	}

	// Instalaciones antiguas: varios logins implican flujo inicial ya superado.
	if err := DB.Model(&models.User{}).Where("login_count >= ?", 3).Updates(map[string]interface{}{
		"first_login_completed":  true,
		"setup_wizard_completed": true,
	}).Error; err != nil {
		return err
	}
	if err := DB.Model(&models.User{}).Where("first_login_completed = ?", true).
		Update("setup_wizard_completed", true).Error; err != nil {
		return err
	}

	if legacyCount > 0 {
		_ = SetConfig(setupWizardCompletedByAPIKey, "1")
	}
	_ = SetConfig(onboardingLegacyMigratedKey, "1")
	return repairInconsistentOnboardingFlags()
}
